...
function doSentSubs (sents, scale, domain)
{
    ...
    prequestion = sents["scales"][scale]["sent_prequestion"];
    ...
    prequestion = prequestion.replace("SP",SP).replace("LS",LS).replace("LS",LS).replace("AA",AA).replace("BB",BB).replace("AB",AB);
    context = context.replace("SP",SP).replace("LS",LS).replace("LS",LS).replace("AA",AA).replace("BB",BB).replace("AB",AB);
    
     return [utterance, question, context, SP, LS, QUD, prequestion];
}
...
var sents = {
    scales: {
		training1: {
		    ...
		    sent_utterance: "'I like watching TV.'",
		    sent_question:  "your friend hates watching TV?",
            sent_prequestion: "From what your friend told you,"
		},	
	   training2: {
            sent_QUD: "What kind of food do you not like?",
		    sent_utterance: "'I don't like eating meat.'",
		    sent_question:  "your friend doesn't like eating beef?",
            sent_prequestion: "From what your friend told you,"
		},	
...
//	   sent_context = sents["domains"][domain]["sent_context"];
	   ...
	   sent_materials = doSentSubs(sents, scale, domain);	
	    ...
	    $("#sent_question").html(sent_materials[6] + " do you think it is possible that " +
				     sent_materials[1]);
